...
async function setStatusItem(itemId, statusAlvo, setorAuth = null) {
  ...
  try {
    ...
    // permite ir para o próximo ou repetir o mesmo (idempotente)
    const esperado = ITEM_TRANSICOES[item.status];
    if (alvo !== item.status && alvo !== esperado) {
      // também aceita pular recebido → concluido se já estava em produção no cliente
      if (!(item.status === 'recebido' && alvo === 'em_producao') &&
          !(item.status === 'em_producao' && alvo === 'concluido') &&
          !(item.status === 'recebido' && alvo === 'concluido')) {
        throw new ErroPedido(
          409,
          `Item em "${item.status}" não pode ir para "${alvo}"`
        );
      }
    }
