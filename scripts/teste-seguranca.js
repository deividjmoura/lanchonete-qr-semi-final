...
async function tPrivilegios() {
  ...
  const forjado = await req('GET', '/api/me', { cookie: 'lqr_session=' + 'a'.repeat(64) });
...
async function tCabecalhos() {
  ...
  if (/__Host-|^lqr_session=/.test(cookie)) ok('nome de cookie de sessão fixo (sem prefixo dinâmico)');
}
