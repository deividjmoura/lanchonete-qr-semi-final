...
function labelItemStatus(s?: string) {
  switch (s) {
    case "recebido":
      return { t: "na fila", tone: "text-stone-400" };
    case "em_producao":
      return { t: "preparando", tone: "text-sky-300" };
    case "concluido":
      return { t: "pronto", tone: "text-lime-300" };
    case "entregue":
      return { t: "entregue", tone: "text-stone-500" };
    default:
      return { t: s || "—", tone: "text-stone-500" };
  }
}
    ...
    >
      ...
      <ul className="mx-auto mt-4 max-w-xs space-y-1.5 rounded-2xl bg-black/35 border border-white/[0.07] p-3.5 text-left">
        {pedido.itens.map((i) => {
          ...
          return (
            <li key={i.id} className="text-[13px] text-stone-200 leading-snug flex items-start justify-between gap-2">
              <span>
                <b className="font-mono text-amber-300">{i.qtd}×</b> {i.nome}
                {i.setor === "bar" && (
                  <span className="ml-1 text-[9px] uppercase tracking-wider text-violet-300">bar</span>
                )}
                {i.removidos.length > 0 && (
                  <span className="block text-[11px] text-rose-300/85">sem {i.removidos.join(", ")}</span>
                )}
              </span>
      ...
      <div className="mt-4 space-y-2">
        ...
        {idsProntos.length > 1 && aindaProducao.length === 0 && (
          <p className="text-[10px] text-stone-500">entrega completa deste pedido</p>
        )}
      </div>
